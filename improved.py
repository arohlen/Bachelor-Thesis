...
weights = {
    ...
    'wd1': tf.get_variable('W5', shape=(4*4*128,128), initializer=tf.contrib.layers.xavier_initializer()),
    'wd2': tf.get_variable('W6', shape=(128,64), initializer=tf.contrib.layers.xavier_initializer()),

    'out': tf.get_variable('W7', shape=(128,n_classes), initializer=tf.contrib.layers.xavier_initializer()),
}
biases = {
    ...
    'bd1': tf.get_variable('B5', shape=(128), initializer=tf.contrib.layers.xavier_initializer()),
    'bd2': tf.get_variable('B6', shape=(64), initializer=tf.contrib.layers.xavier_initializer()),
...
with tf.Session() as sess:
    ...
    for i in range(training_iters):
        ...
        test_acc,valid_loss = sess.run([accuracy,cost], feed_dict={x: test_x[rand:(rand+6000)],y : test_y[rand:(rand+6000)]})
