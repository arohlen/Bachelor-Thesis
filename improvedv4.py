...
def conv_net(x, weights, biases):
    ...
    conv3 = conv2d(conv3, weights['wc6'], biases['bc6'])
...
weights = {
    ...
    'wc4': tf.get_variable('W3', shape=(3,3,64,64), initializer=tf.contrib.layers.xavier_initializer()),
    ...
    'wc7': tf.get_variable('W6', shape=(3,3,128,128), initializer=tf.contrib.layers.xavier_initializer()),

    'wd1': tf.get_variable('W7', shape=(4*4*128,128), initializer=tf.contrib.layers.xavier_initializer()),
    #'wd2': tf.get_variable('W8', shape=(128,64), initializer=tf.contrib.layers.xavier_initializer()),

    'out': tf.get_variable('W9', shape=(128,n_classes), initializer=tf.contrib.layers.xavier_initializer()),
}
biases = {
    ...
    'bc7': tf.get_variable('B6', shape=(128), initializer=tf.contrib.layers.xavier_initializer()),

    'bd1': tf.get_variable('B7', shape=(128), initializer=tf.contrib.layers.xavier_initializer()),
    #'bd2': tf.get_variable('B8', shape=(64), initializer=tf.contrib.layers.xavier_initializer()),

    'out': tf.get_variable('B9', shape=(n_classes), initializer=tf.contrib.layers.xavier_initializer()),
}
...
with tf.Session() as sess:
    ...
    for i in range(training_iters):
        ...
        rand = random.randint(0,6500)
        test_acc,valid_loss = sess.run([accuracy,cost], feed_dict={x: test_x[rand:(rand+3000)],y : test_y[rand:(rand+3000)]})
